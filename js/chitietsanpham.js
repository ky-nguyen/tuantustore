...
window.onload = function () {
    ...
    var tags = ["Samsung", "Toshiba", "Panasonic", "Electrolux", "Aqua", "Sharp", "LG", "Daikin", "Comfee", "Funiki", "Casper", "Tcl", "Gree", "Beko", "Nakagawa", "Midea", "Kagaroo","Hitachi", 
    "Bosh", "Whirlpool"           
];
...
function phanTich_URL_chiTietSanPham() {
    ...
    var s = addThongSo('Loại máy giặt', sanPhamHienTai.detail.screen);
    s += addThongSo('Lồng giặt', sanPhamHienTai.detail.os);
    s += addThongSo('Khối lượng giặt', sanPhamHienTai.detail.camara);
    s += addThongSo('Số người sử dụng', sanPhamHienTai.detail.camaraFront);
    s += addThongSo('Kiểu động cơ', sanPhamHienTai.detail.cpu);
    s += addThongSo('Tốc độ quay vắt tối đa', sanPhamHienTai.detail.ram);
    s += addThongSo('Chất liệu lồng giặt', sanPhamHienTai.detail.rom);
    s += addThongSo('Chất liệu vỏ máy', sanPhamHienTai.detail.microUSB);
    // s += addThongSo('Chất liệu nắp máy:', sanPhamHienTai.detail.ba);
    // s += addThongSo('Chất liệu nắp máy:', sanPhamHienTai.detail.bc);
    // s += addThongSo('Chất liệu nắp máy:', sanPhamHienTai.detail.battery);
    // s += addThongSo('Chất liệu nắp máy:', sanPhamHienTai.detail.battery);
    // s += addThongSo('Chất liệu nắp máy:', sanPhamHienTai.detail.battery);
    // s += addThongSo('Chất liệu nắp máy:', sanPhamHienTai.detail.battery);
    // s += addThongSo('Chất liệu nắp máy:', sanPhamHienTai.detail.battery);
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        default:
            ...
            return `  Giảm 100.000đ ` + span + ` khi thanh toán trực tuyến`;
    }
}
